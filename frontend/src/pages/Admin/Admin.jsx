...
function Admin(){
    ...
    const { data:statsDetail,refetch,isLoading:statsLoading}=    useGetUserChannelProfileQuery();
    const[deleteVideo]=useDeleteVideoMutation();
    const[updateVideo]= useUpdateVideoMutation();
    const [togglepublishStatus]=useTogglePublishStatusMutation();
    const [createVideo]=usePublishAVideoMutation();
    const stats=statsDetail?.data?.[0];
    ...
    const handleSubmitUpload=async(e)=>{
        ...
        for(let key in form ){
            ...
            toast.error(`Failed to upload the videos !${err?.message || ""}`);
        }
    };
    const handleSubmitEdit=async(e,id)=>{
        ...
        try{
            ...
            toast.err(`failed to edit the video !${err?.message || ""}`);
            ...
            <div className="flex min-h-[calc(100vh-66px)] sm:min-h-[calc(100vh-82px)]">
                ...
                {isUploadOpen && (
                    <form onSubmit={ handleSubmitUpload}>
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 onClick=({e}) =>e.stopPropogation()}">
                        <div className="flex items-center justify-between border-b p-4">
                            <h2 className="text-xl font-semibold">upload Videos</h2>
                            <div   className="flex gap-2">
                                <button type="button" onClick={()=>setUploadOpen(false)} className="bg-[#08e6f5] px-3 py-2 font-bold text-black rounded">
                                    <svg
                                    ...
                                    <button type="submit"
                                    className="bg-[#08e6f5] px-3 py-2 font-bold text-black rounded">Save</button>
                                   </div></div>
                                   <div className="mx-auto flex w-full max-w-3xl flex-col gap-y-4 h-auto overflow-y auto">
                                    <div className="w-full border-2 border-dashed px-4 py-12 text-center">
                                        ...
                                        <h6 className="mb-2 font-semibold">Drag and drop video files to upload</h6>
                                          ...
                                          className="group/btn mt-4 inline-flex w-auto cursor-pointer items-center gap-x-2 bg-[#08e6f5] px-3 py-2 text-center font-bold text-black shadow-[5px_5px_5px_0px__0px_#4f4e4e transition-all duration-150 ease-in-out active::translate-x-[5px] active:: translate-y-[5px] active:shadow-[0px_0px_0px_0px_#4f4e4e]">
                                            <input
                                            type="file"
                                            id="videoFile"
                                            name="videoFile"
                                            ...
                                            className="sr-only"/>Select Video
                                          </label>
                                    </div>
