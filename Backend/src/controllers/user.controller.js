...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (avatarLocalPath) {
    avatarUpload = await UploadFile(avatarLocalPath);
  }

  if (coverImageLocalPath) {
    coverUpload = await UploadFile(coverImageLocalPath);
  }
  ...
  res.status(201).json(
    new ApiResponse(201, "User registered successfully ✅", sanitizedUser)
  );
});
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  const options = {
    httpOnly: true,
    secure: false,      // must be false in local development
...
const refreshAccessToken = asyncHandler(async (req, res) => {
 ...
 const options = {
  ...
  sameSite: "None",   // must be None for localhost cross-origin cookies
  path: "/",
};
